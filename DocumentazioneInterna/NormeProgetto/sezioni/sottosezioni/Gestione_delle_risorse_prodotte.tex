...
Per avere una massima organizzazione\ap{G} di tutti i documenti anche i verbali degli incontri hanno un loro specifico codice.
\begin{center}
...
Come supporto per il versionamento si è adottato Git come sistema di versionamento distribuito, usando GitHub per ospitare i repository\{G} del team. Il gruppo potrà interagire con il VCS\{G} sia tramite riga di comando o sia attraverso software dedicati come GitKraken\{G}.

\subsubsection{Repository creati}
Sono state create due repository\{G} per supportare il lavoro del team, quali:
\begin{itemize}
\item \textbf{Stalker}: viene usato per versionare il codice software prodotto dal team. Al momento, tale repository è vuoto, verrà usata in futuro dopo la RR dandogli anche una organizzazione\ap{G} appropriata;
...
Il repository\{G} per la documentazione ha la seguente organizzazione\ap{G} di cartelle:
\begin{itemize}
...
\item Non è permesso effettuare commit\{G} nel ramo master se non attraverso un pull request\{G}  che deve essere accettato dall’amministratore di progetto;
\item Per ogni commit\{G}  è opportuno commentare con chiarezza per capire cosa si è fatto in modo tale da garantire una buona organizzazione\ap{G} e chiarezza nella struttura di versionamento.
\end{itemize}
