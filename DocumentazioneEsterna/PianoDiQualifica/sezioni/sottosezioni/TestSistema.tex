...
TSA3 & Gestione lista delle organizzazioni\ap{G} &
...
\begin{enumerate}
    \item Venga memorizzato il timestamp\ap{G} in cui è avvenuto il movimento\ap{G}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Riceva un messaggio di errore qualora le credenziali LDAP\ap{G} non fossero riconosciute dal server;
    \item Possa inserire il proprio nome utente durante l'autenticazione con le credenziali LDAP\ap{G} aziendali;
    \item Possa inserire la propria password durante l'autenticazione con le credenziali LDAP\ap{G} aziendali.
\end{enumerate} \\
...
\begin{enumerate}
    ...
    \item Riceva un messaggio di errore qualora l'indirizzo IP dell'organizzazione inserito non rappresenti un server LDAP\ap{G};
