...
TSF-X & Accesso all'applicazione di un utente non autenticato\ap{G} con credenziali Stalker & \begin{enumerate}
...
TSF-X & Registrazione nell'applicazione di un utente non autenticato\ap{G} con credenziali Stalker & \begin{itemize}
...
\begin{enumerate}
    \item Verificare che l'amministratore possa monitorare gli accessi degli utenti riconosciuti\ap{G}.
    \item Verificare che l'amministratore possa monitorare gli accessi effettuati da uno specifico utente riconosciuto\ap{G} visualizzandone il nome, cognome e l'orario di accesso.
    \item Verificare che l'amministratore possa filtrare la lista degli accessi\ap{G} di uno specifico utente riconosciuto\ap{G} per data decrescente.
    \item Verificare che l'amministratore possa filtrare la lista degli accessi\ap{G} di uno specifico utente riconosciuto\ap{G} per data crescente.
    \item Verificare che l'amministratore possa filtrare la lista degli accessi\ap{G} di uno specifico utente riconosciuto\ap{G} per una data precisa.
    \item Verificare che l'amministratore possa monitorare gli accessi effettuati presso un luogo da un specifico utente riconosciuto\ap{G} visualizzandone il nome, il cognome e l’orario di accesso.
\end{enumerate} \\
...
\item Verificare che l'utente sia in grado di inserire un'organizzazione\ap{G}, presente nella lista di tutte le organizzazioni,  nella propria lista delle organizzazioni.
\item Verificare che l'utente possa autenticarsi con credenziali LDAP\ap{G}, qualora scaricasse una organizzazione che richieda l'autenticazione con credenziali LDAP\ap{G}.
\item Verificare che l'utente possa rimuovere un'organizzazione\ap{G} dalla propria lista delle organizzazioni preferite.
...
\item Verificare che l'utente possa aggiornare la lista delle organizzazioni tramite temporizzazione\ap{G}.
...
\item Verificare che l'utente possa visualizzare la lista delle organizzazioni ordinate secondo la politica FIFO\ap{G}.
\item Verificare che l'utente possa visualizzare la lista delle organizzazioni che permettono il tracciamento anonimo\ap{G}.
\item Verificare che l'utente possa visualizzare la lista delle organizzazioni che permettono il tracciamento autenticato\ap{G}.
...
\end{enumerate} \\
%R1FA7.1 a R1FA7.3
TSF-X & Autenticazione con credenziali LDAP\ap{G} & \begin{enumerate}
\item Verificare che l'utente anonimo\ap{G} abbia la possibilità di autenticarsi con credenziali aziendali in un'organizzazione che richiede il tracciamento riconosciuto\ap{G}.
\item Verificare che l'utente visualizzi un messaggio di errore qualora le credenziali LDAP\ap{G} non fossero riconosciute dal server.
\item Verificare che l'utente anonimo possa inserire la propria e-mail durante l'autenticazione con le credenziali LDAP\ap{G} aziendali.
\item Verificare che l'utente anonimo possa inserire la propria password durante l'autenticazione con le credenziali LDAP\ap{G} aziendali.
\end{enumerate} \\
% R1FS4.1 a R1FS4.11
TSF-X & Modifica dei dati dell'organizzazione\ap{G} & \begin{enumerate}
\item Verificare che l'amministratore possa inserire un nuovo nome dell'organizzazione\ap{G}.
\item Verificare che l'amministratore possa modificare il nome dell'organizzazione\ap{G}.
\item Verificare che l'amministratore possa inserire una nuova immagine della organizzazione\ap{G}.
\item Verificare che l'amministratore possa modificare l'immagine della organizzazione\ap{G}.
\item Verificare che l'amministratore possa inserire una nuova descrizione dell’organizzazione\ap{G}.
\item Verificare che l'amministratore possa modificare la descrizione dell’organizzazione\ap{G}.
\item Verificare che l'amministratore possa inserire un nuovo indirizzo dell’organizzazione\ap{G}.
\item Verificare che l'amministratore possa modificare l'indirizzo dell’organizzazione\ap{G}.
\item Verificare che l'amministratore possa inserire un nuovo indirizzo IP per l'organizzazione\ap{G}.
\item Verificare che l'amministratore possa modificare l'indirizzo IP dell'organizzazione\ap{G}.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora il nome dell'organizzazione\ap{G} inserito non rispetti i vincoli imposti.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora il nome dell'organizzazione\ap{G} inserito sia già presente nel sistema e associato ad un'altra organizzazione\ap{G}.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora l'immagine dell'organizzazione\ap{G} inserita non rispetti i vincoli imposti.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora la descrizione dell'organizzazione\ap{G} inserita non rispetti i vincoli imposti.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora l'indirizzo dell'organizzazione\ap{G} inserito non rispetti i vincoli imposti.
\item Verificare che l'amministratore visualizzi un messaggio di errore qualora l'indirizzo IP dell'organizzazione\ap{G} inserito non rappresenti un server LDAP\ap{G}.
\item Verificare che l'amministratore possa inviare la richiesta di eliminazione per un'organizzazione\ap{G}.
\item Verificare che l'amministratore possa inserire una motivazione per la richiesta di eliminazione di un'organizzazione\ap{G}.
\item Verificare che l'amministratore possa annullare le modifiche che sta apportando ad una organizzazione\ap{G}.
\end{enumerate} \\
%R1FS8.1 a R1FS8.4
TSF-X & Report tabellare degli accessi ai luoghi dell'organizzazione\ap{G}  & \begin{enumerate}
\item Verificare che l'amministratore possa ricevere un report tabellare degli accesi ai luoghi dell'organizzazione\ap{G}.
\item Verificare che l'amministratore di un'organizzazione a tracciamento autenticato possa generare una tabella delle entrate e uscite degli utenti nei luoghi dell'organizzazione\ap{G}.
\item Verificare che l'amministratore di un'organizzazione a tracciamento autenticato possa generare una tabella delle ore delle ore spese dagli utenti nei luoghi dell'organizzazione\ap{G}.
\item Verificare che l'amministratore di un'organizzazione a tracciamento autenticato o anonimo possa generare una tabella tabella contenente il numero degli utenti e il totale delle ore passate da essi nei luoghi dell’organizzazione\ap{G}. 
\end{enumerate} \\
% fine test Riccardo
...
\begin{enumerate}
    \item Verificare che nel passaggio dalla modalità di tracciamento autenticato\ap{G} a quella anonima venga inviata al sistema la richiesta di uscita dell'utente dal luogo e la successiva richiesta di ingresso di utente anonimo\ap{G}.
    \item Verificare che nel passaggio dalla modalità di tracciamento anonimo\ap{G} a quella autenticata venga inviata al sistema la richiesta di uscita dell'utente dal luogo e la successiva richiesta di ingresso di utente riconosciuto\ap{G}.
\end{enumerate} \\
...
Se l'utente si trova presso un luogo di un'organizzazione\ap{G}:
\begin{enumerate}
    \item Verificare che l'utente possa visualizzare il tempo trascorso all'interno di tutti i luoghi dell'organizzazione\ap{G} dall'ultimo ingresso. %Alla fine non puoi essere tracciato contemporaneamente in due luoghi quindi non credo abbia senso vedere l'uscita precedente ma basta riferirsi al tempo trascorso dall'ultimo accesso
\end{enumerate}
...
% da R1FI2 a R1FS1.3
TSF-X & Accesso al server di un amministratore non autenticato\ap{G} & \begin{enumerate}
    \item Verificare che l'amministratore non autenticato\ap{G} possa inserire l'e-mail correttamente.
    \item Verificare che l'amministratore non autenticato\ap{G} possa inserire correttamente la password.
    \item Verificare che l'amministratore non autenticato\ap{G} visualizzi un messaggio d'errore se l'autenticazione\ap{G} viene negata per inserimento di credenziali errate.
\end{enumerate}
Se l'amministratore non autenticato\ap{G} si è dimenticato della password:
\begin{enumerate}
    \item Verifica che l'amministratore non autenticato\ap{G} possa effettuare il reset della password qualora se la fosse dimenticata.
\end{enumerate} \\
...
TSF-X & Gestione degli amministratori nominati da un amministratore & \begin{enumerate}
    ...
    \item Verificare che l'amministratore proprietario\ap{G} possa modificare i privilegi di un altro amministratore, inserendo il suo indirizzo e-mail.
